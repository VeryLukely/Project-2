...
from tile import Tile, Coin, Tile2, End
...
class Level:
	def __init__(self):
		...
		self.nocollision_sprites = pygame.sprite.Group()
	...
	def setup_level(self):
		for row_index,row in enumerate(LEVEL_MAP):
			for col_index,col in enumerate(row):
				...
				if col == 'Y':
					Tile2((x,y),[self.visible_sprites,self.collision_sprites])
				if col == 'C':
					Coin((x,y),[self.visible_sprites,self.nocollision_sprites])
				if col == 'E':
					End((x, y), [self.visible_sprites, self.nocollision_sprites])
				if col == 'P':
